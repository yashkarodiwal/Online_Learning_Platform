...
const CourseDetail = () => {
    ...
    const fetchProgress = async () => {
        try {
            const res = await axios.get(`http://localhost:5000/api/progress/${id}`, authHeaders);
    ...
    const markComplete = async (lessonId) => {
        try {
            await axios.post(`http://localhost:5000/api/progress/complete/${lessonId}`, {}, authHeaders);
            fetchProgress();
        } catch {
    ...
    const addReply = async (parentId) => {
        ...
        try {
            await axios.post(
                `http://localhost:5000/api/discussions/${id}/reply/${parentId}`,
                { content },
                authHeaders
            );
